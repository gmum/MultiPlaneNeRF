...
from load_multi import load_many_data, get_train_ids, get_test_ids

from multiplane_helpers import EmbeddedMultiImageNeRF, ImageEmbedder
...
def create_mi_nerf(count, args):
    ...
    # Use MultiImage
    model = EmbeddedMultiImageNeRF(None, count).to(device)
    ...
    if args.N_importance > 0:
        model_fine = EmbeddedMultiImageNeRF(None, count).to(device)
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, embedder
...
def train():
    ...
    if args.dataset_type == 'many':
        objects, test_objects, render_poses, hwf = load_many_data(args.datadir)
        print('Loaded many')
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, embedder = create_mi_nerf(100, args)
    global_step = start
   ...
   # poses = torch.Tensor(poses).to(device)
    ...
    if args.render_only:
        ...
        for oi, object in enumerate(test_objects.values()):
            testsavedir = os.path.join(basedir, expname, f'testset_ro', f'{oi}')
        ...
        for oi, object in enumerate(objects.values()):
            testsavedir = os.path.join(basedir, expname, f'trainset_ro', f'{oi}')
            os.makedirs(testsavedir, exist_ok=True)
            poses = object['poses_test']
            render_kwargs_test['network_fn'].image_plane = object['image_plane']
            render_kwargs_test['network_fine'].image_plane = object['image_plane']
    ...
    for i in trange(start, N_iters):
        ...
        for object in objects.values():
            ...
            if use_batching:
                ...
                img_i = np.random.choice(list(range(100)))
                target = object['images'][img_i]
                target = torch.Tensor(target).to(device)
                pose = object['poses'][img_i, :3,:4]
                
                render_kwargs_train['network_fn'].image_plane = object['image_plane']
                render_kwargs_train['network_fine'].image_plane = object['image_plane']
                e = embedder(object['images'][0])
                render_kwargs_train['network_fn'].set_embedding(e)
                render_kwargs_train['network_fine'].set_embedding(e)
