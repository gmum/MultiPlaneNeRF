...
def create_mi_nerf(count, args):
    ...
    # Use MultiImage
    model = MultiImageNeRF(None, count, input_ch_views).to(device)
    ...
    if args.N_importance > 0:
        model_fine = MultiImageNeRF(None, count, input_ch_views).to(device)
...
def config_parser():
    ...
    parser.add_argument("--eval_objects", type=int, default=200, 
                        help='will eval N objects')
...
def eval(eval_dataset, train_set):
    ...
    if args.dataset_type == 'many':
        ...
        dataset = NeRFShapeNetDataset(root_dir='/home/spurek/hyperrf_data/ds', classes=[eval_dataset])
        ...
        dataset_test = NeRFShapeNetDataset(root_dir='/home/spurek/hyperrf_data/ds', classes=[eval_dataset], train=train_set)
        ...
        objects, test_objects, render_poses, hwf = load_many_data(f'/home/spurek/hyperrf_data/ds/{eval_dataset}')
        print('Loaded many')
        near = 2.
        far = 6.
    ...
    eval_results = pd.DataFrame(columns=['class', 'psnr'])
    
    print(f"its me {args.expname}, {eval_dataset} {train_set}")

    objcs_for_training = list(objects.values())
    random.shuffle(objcs_for_training)
    for bi, batch in enumerate(dataloader_test):
        if bi == args.eval_objects:
            break
        ...
        for oi in range(batch['images'].size(0)):
            ...
            psnr = calculate_image_metrics(object, render_kwargs_test, psnr_metric, 5, args)
            #print(f'{bi}: ', psnr.item())
            eval_results= pd.concat([eval_results, pd.DataFrame([{'eval_dataset_class': eval_dataset, 'psnr': psnr.item()}])], ignore_index=True)

    print(eval_results.groupby("class").describe())
    print("---------------")
    print(eval_results[['psnr']].describe())
