...
def train():
    ...
    if args.dataset_type == 'many':
        ...
        dataset_test = NeRFShapeNetDataset(root_dir='/home/spurek/hyperrf_data/ds', classes=[args.dataset_sample], train=True)
    ...
    for bi, batch in enumerate(dataloader_test):
        ...
        results_dir = join(basedir, expname, 'examples_interpolation', str(bi))
        os.makedirs(results_dir, exist_ok=True)
        print(bi)
        for oi in range(batch['images'].size(0)):
            object = {'images': batch['images'][oi].float(), 'cam_poses': batch['cam_poses'][oi].float()}
            object2 = {'images': prev_batch['images'][oi].float(), 'cam_poses': prev_batch['cam_poses'][oi].float()}
            
            frames = []
            for j in range(25):
                catposes = torch.cat([object['cam_poses'][0:j*2], object2['cam_poses'][j*2:50]])
                catimages = torch.cat([object['images'][0:j*2], object2['images'][j*2:50]])
                
                image_plane = ImagePlane(focal, catposes.cpu().numpy(), catimages.cpu().numpy(), 50)
                ...
                with torch.no_grad():
                    ...
                    #Also: pose_spherical(-30, -30, 3.2).to(device)
                    ...
                    frames.append(to8b(img_r.detach().cpu().numpy()))

                    imageio.imsave(join(results_dir, f'mix_{bi}_{j}.png'), to8b(img_r.detach().cpu().numpy()))
                  
            # Animation   
            #writer = imageio.get_writer(join(results_dir, f'mix_{bi}.gif'), duration=50)
            #for frame in frames:
