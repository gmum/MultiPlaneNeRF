...
from load_blender_resized import load_blender_data
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split, orig_hwf, orig_imgs = load_blender_data(args.datadir, args.divide_fac, args.testskip)
        print('Loaded blender', images.shape, render_poses.shape, hwf, args.datadir)
        print('Loaded blender', orig_imgs.shape, orig_hwf)
        ...
        if args.white_bkgd:
            orig_imgs = orig_imgs[...,:3]*orig_imgs[...,-1:] + (1.-orig_imgs[...,-1:])
        else:
            orig_imgs = orig_imgs[...,:3]
    ...
    orig_H, orig_W, orig_focal = orig_hwf
    orig_H, orig_W = int(orig_H), int(orig_W)
    orig_hwf = [orig_H, orig_W, orig_focal]
    ...
    if K is None:
        ...
        orig_K = np.array([
            [orig_focal, 0, 0.5*orig_W],
            [0, orig_focal, 0.5*orig_H],
            [0, 0, 1]
        ])
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_weights==0:
            ...
            #with torch.no_grad():
            #    rgbs, disps = render_path(render_poses, orig_hwf, orig_K, args.chunk, render_kwargs_test)
        ...
        if (i%args.i_testset==0):
            ...
            with torch.no_grad():
                print(orig_hwf)
                render_path(torch.Tensor(poses[i_test]).to(device), orig_hwf, orig_K, args.chunk, render_kwargs_test, gt_imgs=orig_imgs[i_test], savedir=testsavedir)
